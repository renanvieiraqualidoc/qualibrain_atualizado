...
class Relatorio extends BaseController
{
	...
	public function teste() {
			...
			$sheet->setCellValue('C1', 'NOTA N°');
			$sheet->setCellValue('D1', 'CUSTO MÉDIO PONDERADO');
			$rows = 2;
			$db = \Config\Database::connect();
			$products = $db->query("Select sku, data, nro_nota, custo_ponderado from Products_ponderado")->getResult();
			foreach ($products as $val){
					$sheet->setCellValue('A' . $rows, $val->sku);
					$sheet->setCellValue('B' . $rows, $val->data);
					$sheet->setCellValue('C' . $rows, $val->nro_nota);
					$sheet->setCellValue('D' . $rows, $val->custo_ponderado);
					$rows++;
			}
			return $spreadsheet;
			// $spreadsheet = new Spreadsheet();
