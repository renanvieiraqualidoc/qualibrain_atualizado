...
class Pricing extends BaseController
{
	...
	public function margin($data, $model, $margin_view) {
			...
			if(base_url() == 'http://qualibrain.local.com') {
					...
					$response = $client->request('GET', 'http://ultraclinica.totvscloud.com.br:2000/RMS/RMSSERVICES/ReportWebAPI/api/v1/SaleHistory/GetByDate?filial=1007&dataVendaInicio='.date('Y-m-d').'&dataVendaFim='.date('Y-m-d'))->getBody();
			}
